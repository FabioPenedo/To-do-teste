            ...
            app.UseExceptionHandler(appError =>
            {
                appError.Run(async context =>
                {
                    ...
                    if (contextFeature != null)
                    {
                        ...
                        Log.Error(exception,
                            "exception.handled Exception captured while processing request HttpMethod={Method} Path={Path} StatusCode={StatusCode} ExceptionType={ExceptionType} UserId={UserId}",
                            context.Request.Method, context.Request.Path, (int)statusCode, exception.GetType().Name, userId);
